...
void *Video::videoProcess(void *arg) {
    ...
    int texture = video->openGL.init(video->mWindow, NULL, video_dec_ctx->width, video_dec_ctx->height);
    ...
    while (video->thread_flag) {
        ...
        } while (video_packet == NULL && video->thread_flag);
